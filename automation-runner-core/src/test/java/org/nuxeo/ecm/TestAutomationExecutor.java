...
import org.nuxeo.ecm.automation.interactive.op.AutomationExecutor;
...
@Deploy("org.nuxeo.ecm.automation-runner-core")
public class TestAutomationExecutor {
    ...
    @Test
    public void shouldExecJSAndReturnWithFormat() throws Exception {
        OperationContext ctx = new OperationContext(session);
        ctx.setInput(stupidScript);

        String html = (String) automationService.run(ctx, AutomationExecutor.ID);
        ...
        String json = (String) automationService.run(ctx, AutomationExecutor.ID, parameters);
        System.out.println(json);
        JsonNode node = mapper.readTree(json);
        assertTrue(node.has("value"));
        assertTrue(node.has("type"));
        assertTrue(node.has("exec_time"));
    ...
    public void shouldExecJSAndReturnDocRendition() throws Exception {
        ...
        assertEquals("Domain",node.get("docType").asText());
        assertTrue(node.has("exec_time"));
    }
    ...
    public void shouldExecJSAndReturnDocListRenditionJSON() throws Exception {
        ...
        String json = (String) automationService.run(ctx, AutomationExecutor.ID, params);
    ...
    public void shouldRegisterOperation() throws Exception {
        ...
        html = (String) automationService.run(ctx, AutomationExecutor.ID);
        assertTrue(html.contains("Scripting.GetRoot"));
        assertTrue(html.contains("compiled"));
    ...
    @Test
    public void shouldRegisterAndDeregisterAsyncListener() throws Exception {
